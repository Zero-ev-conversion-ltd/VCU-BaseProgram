...
inline uint8_t bitSize(uint64_t v) {
...
inline uint8_t reverse8(uint8_t inputData) {
    inputData = ((inputData * 0x0802LU & 0x22110LU) | (inputData * 0x8020LU & 0x88440LU)) * 0x10101LU >> 16; 
    return inputData;
}

inline uint64_t reverse64(uint64_t inputData) {
    //reverse bytes
...
inline void CAN_encode(CAN_message *msg, double inputDataDouble, uint8_t startBit, uint8_t bitLength, bool bitOrder, bool sign, double Scale, double bias) { //bitOrder is 1 for MSB, 0 for LSB. Signed is 1 for signed, 0 for unsigned
...
inline void CAN_encode(CAN_message *msg, int inputDataInt, uint8_t startBit, uint8_t bitLength, bool bitOrder, bool sign, double Scale, double bias) { //bitOrder is 1 for MSB, 0 for LSB. Signed is 1 for signed, 0 for unsigned
...
inline double CAN_decode(CANMessage *msg, uint8_t startBit, uint8_t bitLength, bool bitOrder, bool sign, double Scale, double bias) { //bitorder is 1 for MSB, 0 for LSB. Signed is 1 for signed, 0 for unsigned
  //Create uint64
  uint64_t dataOut;
